...
//11170:AAMMoJ8PdlsXiNqcXfz2hSLDlZOWGEHrP23
  ...
  if (text === "/start") {
    await bot.sendMessage(chatId, 
      ...
      reply_markup: {
        keyboard: [
          [
            // { text: "🗞️ Инструкция", web_app: { url: webAppUrl2 } },
           
            { text: "🌐 Оплата криптой", web_app: { url: webAppUrl } },
            { text: "📰 Постинг", web_app: { url: webAppUrl } },
          ],

         

          [{ text: "Закрыть", web_app: { url: webAppUrl } }],
        ],
      }
    });
}
...
bot.onText(/\/paybusiness/, async (msg) => {
  ...
  bot.sendMessage(chatId, `<i>Оплатите пакет</i> <b>Бизнес</> <i>по следующей ссылке:</i>${session.url}`,{ parse_mode: 'HTML' });
});
//STRIPE PREMIUM
bot.onText(/\/paypremium/, async (msg) => {
  ...
  bot.sendMessage(chatId, `<i>Оплатите пакет </i><b>Премиум</b><i> по следующей ссылке:</i>${session.url}`,{ parse_mode: 'HTML' });
});
